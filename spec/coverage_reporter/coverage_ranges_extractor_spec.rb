...
RSpec.describe CoverageReporter::CoverageRangesExtractor do
  ...
  context "with AST-based method grouping" do
    let(:temp_dir) { Dir.mktmpdir }
    let(:source_file_path) { File.join(temp_dir, "test_file.rb") }
    let(:relative_filename) { "test_file.rb" }
    ...
    context "when entire method is uncovered" do
      ...
      it "handles method with rescue block" do
        source_code = <<~RUBY
        ...
        expect(result[relative_filename][:actual_ranges]).to contain_exactly([2, 2], [4, 4])
        expect(result[relative_filename][:display_ranges]).to contain_exactly([1, 5])
      end
    end

    context "when method is partially covered" do
      it "does not group method if some lines are covered" do
        source_code = <<~RUBY
    ...
    context "with multiple methods" do
      it "groups each fully uncovered method separately" do
        source_code = <<~RUBY
          def method1
            line1 = 1
          end
      ...
      it "only groups fully uncovered methods" do
        source_code = <<~RUBY
          def covered_method
            line1 = 1
          end
    ...
    context "with class methods" do
      it "groups uncovered class methods" do
        source_code = <<~RUBY
        ...
        coverage_report = {
          "coverage" => {
            relative_filename => { "lines" => [nil, 0, 0, nil] }
          }
        }
        ...
        expect(result[relative_filename][:actual_ranges]).to contain_exactly([2, 3])
        expect(result[relative_filename][:display_ranges]).to contain_exactly([1, 4])
      end
    end

    context "when source file is not available" do
      it "falls back to original behavior" do
        coverage_report = {
          "coverage" => {
    ...
    context "with nested methods" do
      it "groups outer method when all lines including nested method are uncovered" do
        source_code = <<~RUBY
          def outer_method
            def inner_method
              inner_line = 1

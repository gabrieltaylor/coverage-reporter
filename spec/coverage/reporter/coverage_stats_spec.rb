...
require "spec_helper"
require "coverage/reporter/coverage_stats"
  ...
  describe "#initialize" do
    ...
    context "with multiple files and some fully covered" do
      ...
      it "only considers files/lines present in diff and omits fully covered files from uncovered hash" do
        ...
        expect(stats.uncovered.keys).to match_array(["app/models/user.rb", "README.md"])
        expect(stats.uncovered["app/models/user.rb"]).to match_array([6, 7])
        expect(stats.uncovered["README.md"]).to match_array([1, 2])
